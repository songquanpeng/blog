...
func (a *App) baseView(c *gin.Context, title, description, canonicalPath string) (ViewData, error) {
	...
	profileName := option(options, "profile_name", option(options, "author", siteName))
	profileBio := option(options, "profile_bio", option(options, "description", ""))
	profileAvatar := strings.TrimSpace(options["profile_avatar"])
	if profileAvatar == "" {
		profileAvatar = brandImage
	}
	profileSocialLinks := configuredProfileSocialLinks(options)
	...
	data := ViewData{
		...
		ProfileAvatar: publicAssetURL(base, profileAvatar), ProfileSocialLinks: profileSocialLinks,
