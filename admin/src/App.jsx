...
function CLI({ route, notify }) {
  ...
  return <section><h1 className="title">命令行工具</h1><p className="subtitle">通过 CLI 完成页面、文件、站点设置及其他全部后台操作。</p>
    <div className="box"><h2 className="title is-5">安装</h2><p>独立 Go 二进制，无需 Python 或 Go。安装脚本会把 <code>blog-cli</code> 放到 <code>~/.local/bin</code>。</p>{info && <div className="cli-command"><code>{info.installCommand}</code><button className="button is-small" onClick={() => copy(info.installCommand)}>复制</button></div>}<p className="help">安装后运行 <code>blog-cli auth login</code>。非交互环境默认输出 JSON，也可在任意命令中使用 <code>--json</code>。</p></div>
